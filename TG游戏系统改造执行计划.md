...
**原需求：**
- 钱包变更完成时，归档所有活跃节点（status=1 → status=4）
- 新钱包周期从空蛇身开始

**新需求：**
- 钱包变更后**不清空蛇身**，不归档节点
- 新钱包地址的游戏节点**继续拼接蛇身**参与中奖匹配
- **没有"归档状态"概念** - 不需要归档逻辑
- 游戏配置状态停止（`status=0`）时，只是停止监听收款钱包，不再创建新节点

**影响范围：**
1. `TgGameGroupConfigService::completeWalletChange()` - 移除归档节点逻辑
2. `TgPrizeService::checkAndProcessPrize()` - 移除 wallet_cycle 过滤条件
